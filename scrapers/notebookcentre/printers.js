...
const CATEGORY_ID = 172;
const FUNCTION_ID = 501;
...
async function fetchListingPage(page) {
  const { data } = await client.get(LISTING_URL, {
    params: {
      "glkhavvor-fvounkcia": FUNCTION_ID,
...
export async function scrapeNotebookcentrePrinters() {
  let page = 1;
  let allStubs = [];
  ...
  while (page <= MAX_PAGES_SAFETY) {
    try {
      ...
      if (stubs.length === 0) {
        console.log(`[notebookcentre-printers] page ${page} empty — stopping pagination`);
        break;
      }

      console.log(`[notebookcentre-printers] page ${page} -> ${stubs.length} products`);
      allStubs = allStubs.concat(stubs);
      page += 1;
      await sleep(250);
    } catch (err) {
      console.error(`[notebookcentre-printers] failed on page ${page}: ${err.message}`);
      break;
    }
  }
  ...
  console.log(`[notebookcentre-printers] Total: ${results.length}`);
  return results;
}