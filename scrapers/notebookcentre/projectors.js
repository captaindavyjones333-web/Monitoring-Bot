...
export async function scrapeNotebookcentreProjectors() {
  let page = 1;
  let allStubs = [];
  ...
  while (page <= MAX_PAGES_SAFETY) {
    try {
      ...
      if (stubs.length === 0) {
        console.log(`[notebookcentre-projectors] page ${page} empty — stopping pagination`);
        break;
      }

      console.log(`[notebookcentre-projectors] page ${page} -> ${stubs.length} products`);
      allStubs = allStubs.concat(stubs);
      page += 1;
      await sleep(250);
    } catch (err) {
      console.error(`[notebookcentre-projectors] failed on page ${page}: ${err.message}`);
      break;
    }
  }
  ...
  console.log(`[notebookcentre-projectors] Total: ${results.length}`);
  return results;
}