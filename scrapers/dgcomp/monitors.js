...
const LIST_URL =
  "https://dgcomp.am/product-category/%d5%b4%d5%b8%d5%b6%d5%ab%d5%bf%d5%b8%d6%80%d5%b6%d5%a5%d6%80/";
...
async function gotoWithChallengeHandling(page, url, label) {
  ...
  await page
    ...
    .catch(() =>
      console.warn(
        `[dgcomp-monitors] ${label}: .product-name.product_title a never appeared`,
      ),
    );
  ...
  if (title.includes("Just a moment")) {
    ...
    if (!resolved) {
      console.warn(`[dgcomp-monitors] ${label}: challenge never resolved, skipping`);
      return false;
    }
  }
  return true;
...
export async function scrapeDgcompMonitors() {
  ...
  try {
    console.log("[dgcomp-monitors] Loading listing page 1...");
    const ok = await gotoWithChallengeHandling(page, LIST_URL, "page 1");
    if (!ok) return [];

    const page1Products = await extractListingProducts(page);
    console.log(`[dgcomp-monitors] page 1: ${page1Products.length} products`);
    ...
    for (let p = 2; p <= MAX_PAGES && nextExists; p++) {
      ...
      console.log(`[dgcomp-monitors] Loading page ${p}: ${pageUrl}`);
      ...
      if (!pageOk) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[dgcomp-monitors] page ${p}: challenge failed twice in a row, stopping`,
          );
          break;
        }
        continue;
      ...
      console.log(
        `[dgcomp-monitors] page ${p}: ${products.length} products, ${newCount} new`,
      );
      ...
      if (newCount === 0) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[dgcomp-monitors] stopping — 2 consecutive pages with no new products`,
          );
          break;
        }
      } else {
    ...
    console.log(`[dgcomp-monitors] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
