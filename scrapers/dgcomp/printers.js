...
const LIST_URL =
  "https://dgcomp.am/product-category/%d5%bf%d5%ba%d5%ab%d5%b9%d5%b6%d5%a5%d6%80-%d6%87-%d5%ba%d6%80%d5%b8%d5%a5%d5%af%d5%bf%d5%b8%d6%80%d5%b6%d5%a5%d6%80/%d5%bf%d5%ba%d5%ab%d5%b9%d5%b6%d5%a5%d6%80/";
...
async function gotoWithChallengeHandling(page, url, label) {
  ...
  await page
    ...
    .catch(() =>
      console.warn(
        `[dgcomp-printers] ${label}: .product-name.product_title a never appeared`,
      ),
    );
  ...
  if (title.includes("Just a moment")) {
    ...
    if (!resolved) {
      console.warn(`[dgcomp-printers] ${label}: challenge never resolved, skipping`);
      return false;
    }
  }
  return true;
...
export async function scrapeDgcompPrinters() {
  ...
  try {
    console.log("[dgcomp-printers] Loading listing page 1...");
    const ok = await gotoWithChallengeHandling(page, LIST_URL, "page 1");
    if (!ok) return [];

    const page1Products = await extractListingProducts(page);
    console.log(`[dgcomp-printers] page 1: ${page1Products.length} products`);
    ...
    for (let p = 2; p <= MAX_PAGES && nextExists; p++) {
      ...
      console.log(`[dgcomp-printers] Loading page ${p}: ${pageUrl}`);
      ...
      if (!pageOk) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[dgcomp-printers] page ${p}: challenge failed twice in a row, stopping`,
          );
          break;
        }
        continue;
      ...
      console.log(
        `[dgcomp-printers] page ${p}: ${products.length} products, ${newCount} new`,
      );
      ...
      if (newCount === 0) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[dgcomp-printers] stopping — 2 consecutive pages with no new products`,
          );
          break;
        }
      } else {
    ...
    console.log(`[dgcomp-printers] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
