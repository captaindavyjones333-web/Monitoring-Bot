...
const LIST_URL =
  "https://www.zigzag.am/am/tv-audio-video/acoustics-and-speakers.html?cat=1929";

// Target brands — only keep products matching at least one.
const BRAND_REGEX =
  /\bbose\b|\bbeats\b|\bsony\b|\byandex\b|станция|\bxiaomi\b|\bjbl\b|harman|marshall/i;
...
async function waitForChallengeResolution(page, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.warn(
      `[zigzag-speakers] Still on challenge page (attempt ${attempt + 1}/${maxAttempts})...`,
    );
...
async function gotoWithChallengeHandling(page, url, label) {
  ...
  if (title.includes("Just a moment")) {
    ...
    if (!resolved) {
      console.warn(
        `[zigzag-speakers] ${label}: challenge never resolved, skipping`,
      );
      return false;
    }
  }
...
export async function scrapeZigzagSpeakers() {
  ...
  try {
    console.log("[zigzag-speakers] Loading listing page 1...");
    const ok = await gotoWithChallengeHandling(page, LIST_URL, "page 1");
    if (!ok) return [];

    const page1Products = await extractListingProducts(page);
    console.log(`[zigzag-speakers] page 1: ${page1Products.length} products`);
    ...
    for (let p = 2; p <= MAX_PAGES; p++) {
      ...
      console.log(`[zigzag-speakers] Loading page ${p}: ${pageUrl}`);
      ...
      if (!pageOk) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[zigzag-speakers] page ${p}: challenge failed twice in a row, stopping`,
          );
          break;
        }
        continue;
      ...
      console.log(
        `[zigzag-speakers] page ${p}: ${products.length} products, ${newCount} new`,
      );
      ...
      if (newCount === 0) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[zigzag-speakers] stopping — 2 consecutive pages with no new products`,
          );
          break;
        }
      } else {
    ...
    console.log(
      `[zigzag-speakers] After brand filter: ${filtered.length} / ${listingProducts.length}`,
    );
    ...
    console.log(`[zigzag-speakers] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
