...
const LIST_URL =
  "https://www.zigzag.am/am/computers-notebooks-tablets/monitors-new.html?manufacturer=5417";
...
async function gotoWithChallengeHandling(page, url, label) {
  ...
  await page
    ...
    .catch(() =>
      console.warn(`[zigzag-monitors] ${label}: .product_name never appeared`),
    );
  ...
  if (title.includes("Just a moment")) {
    ...
    if (!resolved) {
      console.warn(`[zigzag-monitors] ${label}: challenge never resolved, skipping`);
      return false;
    }
  }
  return true;
...
export async function scrapeZigzagMonitors() {
  ...
  try {
    console.log("[zigzag-monitors] Loading listing page 1...");
    const ok = await gotoWithChallengeHandling(page, LIST_URL, "page 1");
    if (!ok) return [];

    const page1Products = await extractListingProducts(page);
    console.log(`[zigzag-monitors] page 1: ${page1Products.length} products`);
    ...
    for (let p = 2; p <= MAX_PAGES; p++) {
      ...
      console.log(`[zigzag-monitors] Loading page ${p}: ${pageUrl}`);
      ...
      if (!pageOk) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[zigzag-monitors] page ${p}: challenge failed twice in a row, stopping`,
          );
          break;
        }
        continue;
      ...
      console.log(
        `[zigzag-monitors] page ${p}: ${products.length} products, ${newCount} new`,
      );
      ...
      if (newCount === 0) {
        ...
        if (consecutiveNoNew >= 2) {
          console.log(
            `[zigzag-monitors] stopping — 2 consecutive pages with no new products`,
          );
          break;
        }
      } else {
    ...
    console.log(`[zigzag-monitors] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
