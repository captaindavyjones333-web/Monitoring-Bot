...
const LIST_URL = "https://eldorado.am/am/dyson-products-eldorado/dyson-hair-care-armenia-eldorado";
...
async function waitForChallengeResolution(page, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.warn(`[eldorado-dyson] Still on challenge page, waiting (attempt ${attempt + 1}/${maxAttempts})...`);
    await new Promise((r) => setTimeout(r, 4000));
  }
  return false;
}

export async function scrapeEldoradoDyson() {
  ...
  try {
    console.log("[eldorado-dyson] Loading page...");
    ...
    if (title.includes("Just a moment")) {
      ...
      if (!resolved) {
        console.warn("[eldorado-dyson] Challenge never resolved, returning empty");
        return [];
      }
    }

    const products = await extractProducts(page);
    console.log(`[eldorado-dyson] Total: ${products.length}`);
    return products;
  } finally {
    await browser.close();
  }
