...
const LIST_PATH = "/am/photo-and-digital-equipments/photo-camers-and-video-cameras/compact-photo-cameras";
...
async function waitForChallengeResolution(page, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.warn(
      `[eldorado-camera] Still on challenge page, waiting (attempt ${attempt + 1}/${maxAttempts})...`,
    );
...
export async function scrapeEldoradoCamera() {
  ...
  try {
    ...
    while (pageNum <= MAX_PAGES) {
      ...
      console.log(`[eldorado-camera] Loading page ${pageNum}...`);
      ...
      if (title.includes("Just a moment")) {
        ...
        if (!resolved) {
          console.warn(
            `[eldorado-camera] Challenge never resolved on page ${pageNum}, stopping`,
          );
          break;
        }
      }
      ...
      if (pageProducts.length === 0) {
        console.log(
          `[eldorado-camera] Page ${pageNum}: 0 products, stopping`,
        );
        break;
      }
      ...
      console.log(
        `[eldorado-camera] Page ${pageNum}: ${pageProducts.length} products, ${newCount} new`,
      );
      ...
      if (newCount === 0) {
        ...
        if (consecutiveNoNewProducts >= 2) {
          console.log(
            "[eldorado-camera] Stopping — server is repeating content",
          );
          break;
        }
      } else {
    ...
    console.log(`[eldorado-camera] Total: ${result.length}`);
    return result;
  } finally {
    await browser.close();
  }
...
export const scrapeEldoradoCameras = scrapeEldoradoCamera;
