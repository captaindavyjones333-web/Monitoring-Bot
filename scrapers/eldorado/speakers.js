...
async function waitForChallengeResolution(page, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.warn(
      `[eldorado-speakers] Still on challenge page, waiting (attempt ${attempt + 1}/${maxAttempts})...`,
    );
...
export async function scrapeEldoradoSpeakers() {
  ...
  try {
    ...
    while (pageNum <= MAX_PAGES) {
      ...
      if (title.includes("Just a moment")) {
        ...
        if (!resolved) {
          console.warn(
            `[eldorado-speakers] Challenge never resolved on page ${pageNum}, stopping`,
          );
          break;
        }
      }
      ...
      if (pageProducts.length === 0) {
        console.log(
          `[eldorado-speakers] Page ${pageNum}: 0 products, stopping`,
        );
        break;
      }
      ...
      console.log(
        `[eldorado-speakers] Page ${pageNum}: ${pageProducts.length} products, ${newCount} new`,
      );

      if (newCount === 0) {
        consecutiveNoNewProducts++;
        console.log(
          `[eldorado-speakers] No new products (${consecutiveNoNewProducts} consecutive), likely repeating last page`,
        );
        if (consecutiveNoNewProducts >= 2) {
          console.log(
            "[eldorado-speakers] Stopping — server is repeating content",
          );
          break;
        }
      } else {
    ...
    console.log(`[eldorado-speakers] Total: ${result.length}`);
    return result;
  } finally {
    await browser.close();
  }
