...
const LIST_URL =
  "https://mobilecentre.am/category/computers/144/0/?search=filters&searchData_brand=%5B%2255413%22%5D&searchData_TAG_100=%5B%22Notebook%22%5D";
...
async function getAllProductUrls(page) {
  ...
  while (noChangeCount < 3) {
    ...
    console.log(`[mc-macbooks] Items loaded: ${count}`);
...
async function fetchProductDetail(page, listingName, url, listingCashPrice) {
  try {
    ...
    if (/error\s*100\d/i.test(fullName) || fullName.length < 3) {
      console.warn(`[mc-macbooks] Blocked/error page at ${url}, skipping`);
      return null;
    }
...
export async function scrapeMobileCentreMacbooks() {
  ...
  try {
    console.log("[mc-macbooks] Loading listing page...");
    const listingProducts = await getAllProductUrls(page);
    console.log(`[mc-macbooks] ${listingProducts.length} products found`);
    ...
    for (let i = 0; i < listingProducts.length; i++) {
      ...
      console.log(`[mc-macbooks] (${i + 1}/${listingProducts.length}) ${detail?.name}`);
      await new Promise((r) => setTimeout(r, 600));
    }

    console.log(`[mc-macbooks] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
