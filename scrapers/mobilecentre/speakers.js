...
const LIST_URL =
  "https://mobilecentre.am/category/accessories/146/0/?search=filters&searchData_brand=%5B%2256076%22%2C%2256057%22%2C%2255839%22%5D&searchData_cats=%5B%22158%22%5D";
...
async function getAllProductUrls(page) {
  ...
  while (noChangeCount < 3) {
    ...
    console.log(`[mc-speakers] Items loaded: ${count}`);
...
async function fetchProductDetail(page, listingName, url, listingCashPrice) {
  try {
    ...
    if (/error\s*100\d/i.test(fullName) || fullName.length < 3) {
      console.warn(`[mc-speakers] Blocked/error page at ${url}, skipping`);
      return null;
    }
    ...
    console.warn(`[mc-speakers] Failed ${url}: ${err.message}`);
    return null;
  }
}

export async function scrapeMobileCentreSpeakers() {
  ...
  try {
    console.log("[mc-speakers] Loading listing page...");
    const listingProducts = await getAllProductUrls(page);
    console.log(`[mc-speakers] ${listingProducts.length} products found`);
    ...
    for (let i = 0; i < listingProducts.length; i++) {
      ...
      console.log(`[mc-speakers] (${i + 1}/${listingProducts.length}) ${detail?.name}`);
      await new Promise((r) => setTimeout(r, 600));
    }

    console.log(`[mc-speakers] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
