...
async function getAllProductUrls(page) {
  ...
  while (noChangeCount < 3) {
    ...
    console.log(`[mc-headphones] Items loaded: ${count}`);
...
async function fetchProductDetail(page, listingName, url, listingCashPrice) {
  try {
    ...
    if (/error\s*100\d/i.test(fullName) || fullName.length < 3) {
      console.warn(`[mc-headphones] Blocked/error page at ${url}, skipping`);
      return null;
    }
    ...
    console.warn(`[mc-headphones] Failed ${url}: ${err.message}`);
    return null;
  }
}

export async function scrapeMobileCentreHeadphones() {
  ...
  try {
    console.log("[mc-headphones] Loading listing page...");
    const listingProducts = await getAllProductUrls(page);
    console.log(`[mc-headphones] ${listingProducts.length} products found`);
    ...
    for (let i = 0; i < listingProducts.length; i++) {
      ...
      console.log(`[mc-headphones] (${i + 1}/${listingProducts.length}) ${detail?.name}`);
      await new Promise((r) => setTimeout(r, 600));
    }

    console.log(`[mc-headphones] Total: ${results.length}`);
    return results;
  } finally {
    await browser.close();
  }
