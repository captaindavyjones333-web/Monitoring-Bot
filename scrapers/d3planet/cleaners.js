...
const LIST_URL = "https://3dplanet.am/hy/store/vacuum-cleaners";
...
async function fetchListing() {
  ...
  console.log(`[3d-cleaners] ${totalPages} pages`);

  const listingProducts = [];
  for (let page = 1; page <= totalPages; page++) {
    const products = await fetchListingPage(LIST_URL, page);
    console.log(`[3d-cleaners] Page ${page}: ${products.length} products`);
...
async function fetchProductPrice(puppeteerPage, baseName, url) {
  try {
    ...
    console.warn(`[3d-cleaners] Failed ${url}: ${err.message}`);
    return null;
  }
}

export async function scrape3DPlanetCleaners() {
  const listingProducts = await fetchListing();
  console.log(`[3d-cleaners] ${listingProducts.length} unique products found`);
  ...
  try {
    for (let i = 0; i < listingProducts.length; i++) {
      ...
      console.log(`[3d-cleaners] (${i + 1}/${listingProducts.length}) ${name}`);
  ...
  console.log(`[3d-cleaners] Total: ${results.length}`);
  return results;
}
