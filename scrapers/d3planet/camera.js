...
const LIST_URL = "https://3dplanet.am/hy/store/photo-cameras";
...
async function fetchListing() {
  ...
  console.log(`[3d-camera] ${totalPages} pages`);

  const listingProducts = [];
  for (let page = 1; page <= totalPages; page++) {
    const products = await fetchListingPage(LIST_URL, page);
    console.log(`[3d-camera] Page ${page}: ${products.length} products`);
...
async function fetchProductPrice(puppeteerPage, baseName, url) {
  try {
    ...
    console.warn(`[3d-camera] Failed ${url}: ${err.message}`);
    return null;
  }
}

export async function scrape3DPlanetCamera() {
  const listingProducts = await fetchListing();
  console.log(`[3d-camera] ${listingProducts.length} unique products found`);
  ...
  try {
    for (let i = 0; i < listingProducts.length; i++) {
      ...
      console.log(`[3d-camera] (${i + 1}/${listingProducts.length}) ${name}`);
  ...
  console.log(`[3d-camera] Total: ${results.length}`);
  return results;
}

export const scrape3DPlanetCameras = scrape3DPlanetCamera;
