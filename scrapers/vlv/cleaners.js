...
const URL = "https://v1.vlv.am/api/category/vacuum-cleaner";
...
async function fetchAllPages() {
  ...
  while (true) {
    const res = await axios.post(
      URL,
      new URLSearchParams({ slug: "vacuum-cleaner", page: String(page) }),
    ...
    console.log(
      `[vlv-cleaners] Page ${page}/${lastPage}: ${products.length} products`,
    );
...
export async function scrapeVlvCleaners() {
  const raw = await fetchAllPages();
  const results = raw.map(normalize).filter((p) => p.name && p.cash_price);
  console.log(`[vlv-cleaners] Total: ${results.length}`);
  return results;
}
